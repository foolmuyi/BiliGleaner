...
export async function persistUpdateCheckResult(mid, lastBvid, updates = [], checkedAt = Date.now()) {
  const normalizedMid = String(mid);
  const entries = Array.isArray(updates) ? updates : [];
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction([STORES.TRACKING, STORES.UPDATES], 'readwrite');
    const trackingStore = tx.objectStore(STORES.TRACKING);
    const updatesStore = tx.objectStore(STORES.UPDATES);
    let trackedExists = false;

    const trackingReq = trackingStore.get(normalizedMid);
    trackingReq.onsuccess = () => {
      const existing = trackingReq.result;
      if (!existing) return;
      ...
      trackingStore.put({
        ...existing,
        mid: normalizedMid,
        ...(lastBvid ? { last_bvid: lastBvid } : {}),
        last_checked_at: Number(checkedAt) || Date.now(),
      });

      for (const entry of entries) {
        const getReq = updatesStore.get(entry.id);
        getReq.onsuccess = () => {
          const existingEntry = getReq.result;
          updatesStore.put(existingEntry?.is_read === 1 ? { ...entry, is_read: 1 } : entry);
        };
        getReq.onerror = () => reject(getReq.error);
      }
    };
    trackingReq.onerror = () => reject(trackingReq.error);
    tx.oncomplete = () => resolve(trackedExists);
    tx.onerror = () => reject(tx.error);
  });
}
